...
$("#loadB").click(function(){
	...
	function loadBook()
	{
		...
		sessionStorage.setItem("mail", username);
         ...
         getDeleveryManId(username,password);
		$.ajax({
			type: 'GET',
			...
			complete:function(xmlHttp,status){
				if(xmlHttp.status==200)
				{
					...
					if(statusu==2)
					{
						...
						window.location.href = "/ATP2_Final_API/Deliveryman/Dashboard.html";
					}
	...
	function getDeleveryManId(username,password)
	{
						$.ajax({
								type: 'GET',
                    
                    			url: 'https://localhost:44327/api/deleverymen',
                    			dataType: 'json',
								...
								complete:function(xmlHttp,status){
									if(xmlHttp.status==200)
									{
										...
										for (var i = 0; i < data.length; i++) {
											if(data[i].email==username){
												sstr=data[i].deleveryManId;
												sessionStorage.setItem("did", sstr);
